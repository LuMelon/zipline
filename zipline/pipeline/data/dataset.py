...
class MultiDimensionalDataSetMeta(abc.ABCMeta):
    ...
    def __new__(cls, name, bases, dict_):
        ...
        try:
            existing_columns = self._columns
        ...
        if not is_base_class:
            self.free_axes = free_axes = OrderedDict([
                (k, frozenset(v)) for k, v in self.free_axes
            ])
            if not free_axes:
                raise ValueError(
...
class MultiDimensionalDataSet(_base):
    ...
    def slice(cls, *args, **kwargs):
        ...
        **free_axes_labels
        ...
        class Slice(MultiDimensionalDataSetSlice):
            parent_multidimensional_dataset = cls
            free_axes_labels = labels

            ndim = cls.ndim

            locals().update(cls._columns)
